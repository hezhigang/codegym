...
public class MeisselLehmerPrimeCounting {
    ...
    final static int PM = 2 * 3 * 5;

    static int[][] phi = new int[PM + 1][M + 1];

    static int[] sz = new int[M + 1];
    ...
    static long getphi(long x, int s) {
        if (s == 0)
            return x;
        if (s <= M)
    ...
    public static void main(String[] args) {
        long n = 100000L;
        init();
        long pi_n = lehmer_pi(n);
        System.out.printf("the count of primes less than %d = %d", n, pi_n);
        System.out.println();
    }
}